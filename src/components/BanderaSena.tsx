...
import banderaSenaImg from '../assets/images/bandera_sena_1790256947477.jpg';
import { ZoomIn, X, Flag } from 'lucide-react';

interface BanderaSenaProps {
  compact?: boolean;
  className?: string;
  showDetails?: boolean;
}

export const BanderaSena: React.FC<BanderaSenaProps> = ({
  compact = false,
  className = '',
  showDetails = true
}) => {
  ...
  return (
    <div className={`flex flex-col items-center ${className}`}>
      ...
      {/* Contenedor Visual de la Bandera Ondulante con Asta */}
      <div
        ...
        title="Haga clic para ampliar la Bandera Oficial del SENA"
      >
        <div className="relative w-full max-w-[190px] aspect-[4/3] flex items-center justify-center">
          <img
            src={banderaSenaImg}
            alt="Bandera Oficial del SENA ondeando"
        ...
        <div className="mt-2 text-center">
          <span className="text-[10px] font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full inline-block">
            Pabellón de Paz y Transparencia
          </span>
        </div>
      </div>

      {/* Modal Zoom de la Bandera */}
      {showZoom && (
        <div className="fixed inset-0 z-50 bg-slate-900/75 backdrop-blur-xs flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl max-w-md w-full overflow-hidden shadow-2xl border border-slate-200 relative animate-scaleUp">
            ...
            <div className="p-6 border-b border-slate-100 bg-slate-50 text-center">
              ...
              <h3 className="text-xl font-black text-slate-900 mt-1">Bandera Oficial del SENA</h3>
              <p className="text-xs text-slate-500">Pabellón de lienzo blanco con el escudo institucional</p>
            </div>

            {/* Imagen ampliada */}
            <div className="p-6 flex flex-col items-center bg-white">
              <div className="w-64 h-64 max-w-full flex items-center justify-center">
                <img
                  src={banderaSenaImg}
                  alt="Bandera Oficial del SENA ampliada"
                  className="w-full h-full object-contain filter drop-shadow-xl"
                  referrerPolicy="no-referrer"
                />
              </div>

              {/* Guía heráldica y significado */}
              <div className="w-full mt-6 space-y-2 text-xs">
                <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200 flex items-center space-x-2 text-slate-800">
                  <span className="text-lg">🏳️</span>
                  <div>
                    <span className="font-bold">Lienzo Blanco:</span> Simboliza la paz, la tranquilidad, la pulcritud y la transparencia en la formación integral.
                  </div>
                </div>
                ...
                <div className="bg-emerald-50 p-2.5 rounded-xl border border-emerald-200 flex items-center space-x-2 text-emerald-950">
                  <span className="text-lg">🛡️</span>
                  <div>
                    <span className="font-bold">Emblema Central:</span> Integra los tres sectores económicos fundamentales de Colombia (campo, industria y comercio).
                  </div>
                </div>

                <div className="bg-amber-50 p-2.5 rounded-xl border border-amber-200 flex items-center space-x-2 text-amber-950">
                  <span className="text-lg">🚩</span>
                  <div>
                    <span className="font-bold">Asta y Firmeza:</span> La soberanía formativa y el compromiso de la entidad con el desarrollo de la nación.
                  </div>
                </div>
              </div>
            </div>
